...
public class Battlefield {
    ...
    public boolean putShipOnField(int _rowBegin, int _columnBegin, int _rowEnd, int _columnEnd, Ship _ship){
            ...
            if(ship != _ship && ship.isPlaced()){
                for(int i = _rowBegin -1; i <= _rowEnd +1; i++){
                    for(int j = _columnBegin -1; j<= _columnEnd +1; j++){
                        if((i == ship.getRowBegin() && j == ship.getColumnBegin()) || i == ship.getRowEnd() && j == ship.getColumnEnd()){
        ...
        if(_rowBegin == _rowEnd){
            for(int i = _columnBegin; i<= _columnEnd; i++){
                this.gamefield[_rowBegin][i-1] = _ship.getCells()[i - _columnBegin];
            }
        }else{
            for(int i =_rowBegin; i<= _rowEnd; i++){
                this.gamefield[i][_columnBegin -1] = _ship.getCells()[i - _rowBegin];
            }
        }
        return true;
    }
    ...
    public boolean makeShot(){
        ...
        if(shotRow <0 || shotRow>9 || shotColumn <1 || shotColumn> 10){
            ...
            if(isHitOnShip && !isSankAShip){
                ...
                this.gamefield[shotRow][shotColumn-1] = MISSED;
        ...
        for(Ship ship: ships){
            if(ship.rowBegin == ship.rowEnd){
                for(int i = ship.columnBegin; i<= ship.columnEnd; i++){
                    if(_fogOfWar){
                        this.gamefield[ship.rowBegin][i-1] = ship.getCells()[i - ship.columnBegin] == SHIP ? EMPTY : ship.getCells()[i - ship.columnBegin];
                    }else{
                        this.gamefield[ship.rowBegin][i-1] = ship.getCells()[i - ship.columnBegin];
                    }
                }
            }else{
                for(int i = ship.rowBegin; i <= ship.rowEnd; i++){
                    if(_fogOfWar){
                        this.gamefield[i][ship.columnBegin-1] = ship.getCells()[i - ship.rowBegin] == SHIP ? EMPTY : ship.getCells()[i - ship.rowBegin];
                    }else{
                        this.gamefield[i][ship.columnBegin-1] = ship.getCells()[i - ship.rowBegin];
                    }
                }
            }
        }
    ...
    public String toString(){
        ...
        for(int i = 0; i< SIZE; i++){
            ...
            for(int j = 0; j< SIZE; j++){
                result.append(" ").append(gamefield[i][j]);
            }
            result.append("\n");
        }
